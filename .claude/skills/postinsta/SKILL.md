...
- **Centrage vertical** : le bloc de texte doit toujours être centré verticalement au milieu du canvas, pas collé en haut. Les templates de liste (`statistique`, `programme`, `classement`) mesurent désormais la hauteur totale de leur contenu et le centrent automatiquement (helper `centeredStartY` dans `macro-instagram.js`). Les slides avec photo de fond (`post-image`) gardent volontairement leur texte en bas sur le dégradé. Si un nouveau template de texte ancre son bloc en haut (`let y = H*0.0X + ...`), le passer sur `centeredStartY(H, blockH, scale)` après avoir mesuré `blockH`.
